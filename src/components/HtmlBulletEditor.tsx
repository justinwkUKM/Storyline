...
export function HtmlBulletEditor({ 
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-3 py-1.5 bg-gray-50/70 border-b border-gray-100 flex-wrap gap-2 select-none">
          ...
          {!isSourceMode ? (
            <>
              ...
              <button
                ...
                className="p-1.5 rounded hover:bg-gray-200/80 text-gray-700 transition-colors"
                title="Bold"
              >
              ...
              <div className="relative">
                <button
                  ...
                  className="p-1.5 rounded hover:bg-gray-200/80 text-gray-700 transition-colors flex items-center gap-0.5"
                  title="Text Color"
                >
                ...
                {showColorPicker && (
                  <div className="absolute left-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-xl p-2 z-50 flex flex-col gap-1 min-w-[120px]">
                    <span className="text-[9px] font-bold text-gray-400 uppercase tracking-widest px-1 pb-1 border-b border-gray-100">Colors</span>
                    {PRESET_COLORS.map((col) => (
                      <button
                        key={col.name}
                        type="button"
                        onClick={() => applyColor(col.value)}
                        className="text-left px-2 py-1 text-xs rounded hover:bg-gray-100 flex items-center gap-2 transition-colors cursor-pointer"
                      >
                        <span 
        ...
        <button
          ...
          className={cn(
            "px-2.5 py-1 rounded text-[10px] font-bold flex items-center gap-1 cursor-pointer transition-all border",
            isSourceMode
      ...
      <div className="relative min-h-[48px] flex items-stretch">
        {!isSourceMode ? (
          ...
          <div
            ...
            className={cn(
              "flex-1 px-4 py-2.5 text-sm outline-none whitespace-pre-wrap leading-relaxed prose prose-sm max-w-none text-gray-700 min-h-[48px] select-text",
              value === '' && "before:content-[attr(placeholder)] before:text-gray-400 before:pointer-events-none before:absolute"
            )}
          ...
          <input
            ...
            className="flex-1 w-full px-4 py-2.5 text-sm text-gray-800 font-mono bg-slate-50/50 outline-none min-h-[48px]"
          />
        )}
      </div>
      ...
      {isFocused && (
        <div className="px-3 py-1 bg-gray-50 border-t border-gray-100 text-[10px] text-gray-400 font-medium flex items-center gap-1 select-none">
